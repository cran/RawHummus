...
sumTIC <- msdata$TIC[, list(peakArea = sum(int)), by = list(filename)]
...
In order to more accurately monitor variations in mass, RT and ion intensity, <span style = "color : #FF8300">**RawHummus**</span> automatically selects **6** peaks across the entire RT range, and use them to evaluate LCMS system.
...
##############################################################################
  ...
  .[, `:=`("Max RT Diff (min)" =  round(max(rt) - min(rt), 2),
           "Max Mass Diff (ppm)" = round((max(mz) - min(mz))/min(mz) * 10^6, 2),
           "Max Intensity Ratio" = round(max(int) / min(int), 2)), by = id] %>%
  mutate(Peak = id) %>%
  select(Peak, "Max RT Diff (min)", "Max Mass Diff (ppm)", "Max Intensity Ratio") %>%
  distinct_all() %>%
  mutate("Max RT Diff (min)" = kableExtra::cell_spec(`Max RT Diff (min)`, 
...
- In **data-dependent acquisition (DDA)** mode, the MS detects the abundance of mass features in MS1 and selects top N features based on intensity for fragmentation; theoretically the number of selected precursors for fragmentation at each scan (or RT) should be the same. If this mode was used for your data acquisition, you are expected to see a homogeneous precursor ions distribution across the entire RT range.

- Commonly used **data-independent acquisition (DIA)** mode includes all-ion fragmentation (AIF) (e.g., MS<sup>All</sup> and MS<sup>E</sup>), where all precursors are fragmented; and sequential window acquisition of all theoretical fragment-ion spectra (SWATH), where a medium pass window, such as 20 or 25 Da, is used [3]. If this mode was used for your data acquisition, you are expected to see a continuous precursor ion distribution across the entire RT range.
...
[3] Guo, J., & Huan, T., 2020. Comparison of full-scan, data-dependent, and data-independent acquisition modes in liquid chromatography–mass spectrometry based untargeted metabolomics. Analytical chemistry, 92(12), pp.8072-8080.
